"""Matriz de sensibilidade de tensao dV/dP obtida numericamente do OpenDSS.
...
CONVENCAO DE SINAL
------------------
`S[i, j] = dV_i / dP_j`, com P em INJECAO (positivo = gera, tensao sobe), V em pu
e P em kW. O modelo do DSO trabalha com potencia de CARGA, entao la a matriz
entra com sinal trocado (`-S`), do mesmo jeito que o codigo original usa
`-jac_inv_21`.
...
def compute_day(dss, nodes, loads_by_period, delta_kw=DEFAULT_DELTA_KW,
    ...
    for loads in loads_by_period:
        v0, s = compute_sensitivity(dss, nodes, loads, delta_kw, central, pf)
        v0_list.append(v0)
        s_list.append(s)
    return np.array(v0_list), np.array(s_list)
...
def _run_day(args):
    ...
    v0, s = compute_day(dss, nodes, periods)

    np.savez(args.out, nodes=np.array(nodes), v0=v0, s=s)
    below = (v0 < 0.97).sum()
    above = (v0 > 1.03).sum()
...
def _main():
    ...
    t0 = time.time()
    v0, s = compute_sensitivity(dss, nodes, base, delta_kw=DEFAULT_DELTA_KW, central=True)
    dt = time.time() - t0
    ...
    # parametrizadas por padrao.
    print("\ncusto da hipotese dQ=0 se o dispositivo mantiver fator de potencia:")
    print(f"{'|dP| por no':>14} {'erro max':>12} {'erro rms':>12}")
    for amp in (1.0, 5.0):
        dp = amp * direction
        v_lin = predict(v0, s, dp)
        loads = dict(base)
        for node in lv_nodes:
            loads[node] = base.get(node, 0.0) - dp[idx[node]]
        apply_loads(dss, loads)
        solve(dss)
        v_real = read_voltages(dss, nodes)
        err = v_lin - v_real
        print(f"{amp:>13.1f} {np.abs(err).max():>12.2e} "
              f"{np.sqrt((err ** 2).mean()):>12.2e}")
    ...
    for delta in (0.1, 0.5, 2.0, 5.0):
        _, s_d = compute_sensitivity(dss, nodes, base, delta_kw=delta, central=True)
        print(f"  delta={delta:>4} kW: |S - S(1 kW)| max = {np.abs(s_d - ref).max():.2e} pu/kW")

    _, s_fwd = compute_sensitivity(dss, nodes, base, delta_kw=DEFAULT_DELTA_KW, central=False)
