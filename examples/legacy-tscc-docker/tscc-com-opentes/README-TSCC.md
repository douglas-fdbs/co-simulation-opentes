...
> Nota OpenTES: nesta integração monorepo, a execução recomendada do TSCC é feita
> pelo `docker-compose.yaml` da raiz do `co-simulation-opentes`. Os Dockerfiles
> antigos das subpastas foram arquivados em `examples/legacy-tscc-docker/tscc-com-opentes`
> e substituídos por um `Dockerfile` multi-stage neste diretório.
